...
class SimbaContractAsync(SimbaContract):
    ...
    async def get_transactions(self, query_args: Optional[dict] = None):
        query_args = query_args or {}
        return await SimbaRequest("v2/apps/{}/transactions/".format(self.contract_uri), query_args).send_async()

    async def validate_bundle_hash(self, bundle_hash: str, query_args: Optional[dict] = None):
        query_args = query_args or {}
        return await SimbaRequest("v2/apps/{}/validate/{}/{}".format(self.app_name, self.contract_name, bundle_hash), query_args).send_async()

    async def get_transaction_statuses(self, txn_hashes: List[str] = None, query_args: Optional[dict] = None):
        ...
        return await SimbaRequest("v2/apps/{}/contract/{}/transactions".format(
            self.app_name, self.contract_name
        ), query_args).send_async()
