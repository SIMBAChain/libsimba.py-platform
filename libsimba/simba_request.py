...
from libsimba.exceptions import SimbaRequestException, SimbaInvalidURLException, LibSimbaException
...
class SimbaRequest:
    ...
    def url(self):
        if self.endpoint is None:
            raise SimbaInvalidURLException(message="SimbaRequest object has no target endpoint")
        else:
            return build_url(SimbaRequest.base_api_url, self.endpoint, self.query_params)
    ...
    @auth_required
    def send(self, headers: dict, json_payload: Optional[dict] = None, files: dict = None, fetch_all: Optional[bool] = True):
        if self.method == 'get':
    ...
    @auth_required
    async def send_async(self, headers: dict, json_payload: Optional[dict] = None, files: dict = None, fetch_all: Optional[bool] = True):
    ...
    def _process_response(self, response: Response, headers: dict, fetch_all: Optional[bool] = False):
    ...
    async def _process_response_async(self, client: httpx.AsyncClient, response: Response, headers: dict, fetch_all: Optional[bool] = False):
        json_response = self._json_response_or_raise(response)
        if fetch_all:
    ...
    async def _fetch_all_async(self, json_response: dict, headers: dict, client: httpx.AsyncClient):
        if not json_response.get('results'):
            return json_response
